...
func TestDecodeBencode(t *testing.T) {
	testCases := []struct {
		...
		{input: "d8:announce55:http://bittorrent-test-tracker.codecrafters.io/announcee", expected: map[string]interface{}{
			"announce": "http://bittorrent-test-tracker.codecrafters.io/announce",
		}, err: nil},

		// Negative test cases
		{input: "invalid", expected: "", err: errInvalidFormat},
		{input: "l5:helloinvalide", expected: "", err: errInvalidNum},
		{input: "di52e3:bar5:helloi52ee", expected: "", err: errInvalidDictKey},
	}

	for _, tc := range testCases {
		t.Run(tc.input, func(t *testing.T) {
			result, _, err := decodeBencode(tc.input)
			if err != nil {
				if tc.err == nil {
